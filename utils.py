...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, cls_num):
    """
    ...
    for i in range(len(det_boxes)):
        det_boxes[i] = torch.Tensor(det_boxes[i])
        det_labels[i] = torch.Tensor(det_labels[i])
        det_scores[i] = torch.Tensor(det_scores[i])
        true_boxes[i] = torch.Tensor(true_boxes[i])
        true_labels[i] = torch.Tensor(true_labels[i])
    ...
    for c in range(1, n_classes):
        ...
        true_class_difficulties = torch.zeros(true_class_boxes.shape[0])
