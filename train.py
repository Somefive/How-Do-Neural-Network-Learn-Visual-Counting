...
from mnist_dataset import MNISTDataset, MNISTCLSDataset
from dot_dataset import DotDataset
from model import MNISTBaseLineModel, DotBaseLineModel
...
dataset   = {'mnist-train'     : MNISTDataset,
             ...
             'mnist-cls-train' : MNISTCLSDataset,
             'mnist-cls-test'  : MNISTCLSDataset,
        #    'dot'   :DotDataset, 
            }
procedure = { 'mnist'    : MNISTBaseLineModel,
              'mnist-cls': MNISTCLSBaseLineModel,
            }
...
# Model
model = procedure[args.dataset](size=args.grid_size * 28).double().to(device)
...
# Generators
Dataset_train = dataset['{:}-train'.format(args.procedure)]
training_set = Dataset_train(10000, grid_size=args.grid_size, confusion=True, target=6)
training_generator = data.DataLoader(training_set, **params)

Dataset_test = dataset['{:}-test'.format(args.procedure)]
validation_set = Dataset_test(1000, grid_size=args.grid_size, confusion=True, target=6)
