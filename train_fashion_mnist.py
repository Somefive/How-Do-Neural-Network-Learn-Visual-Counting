...
log_dir = os.path.join('runs', time_for_file() + '_seed' + str(rand_seed) + '_fashion_mnist' + ("_" + args.cm if args.cm != "" else ""))
...
trainset = F_MNISTDataset(size=args.train_set_size, **args.dataset_params)
...
def run(train_mode=True, epoch=0):
    ...
    for idx, (local_batch, local_labels) in iterator:
        ...
        y_pred, _ = model(local_batch.squeeze())
        y_true = local_labels
